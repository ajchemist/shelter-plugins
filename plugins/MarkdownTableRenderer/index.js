...
function restoreOriginalMessages() {
    for (const [el, html] of this.originals.entries()) {
...
function parseTable(tableText) {
    ...
    const rows = lines.slice(2)
        .filter(line => this.isTableRowLine(line) && !this.isSeparatorLine(line))
        .map(line => this.splitRow(line));
...
function getTextSegments(root) {
    ...
    const visit = node => {
        ...
        if (node.nodeType === Node.TEXT_NODE) {
            if (node.nodeValue) segments.push({ node, text: node.nodeValue, type: "text" });
            return;
        }
        if (node.tagName === "BR") {
...
function findTableRanges(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        spans.push({ start: offset, end: offset + lines[i].length, text: lines[i] });
    ...
    for (let i = 0; i < lines.length - 1; i++) {
        ...
        let endLine = i + 2;
        while (endLine < lines.length && this.isTableRowLine(lines[endLine]) && !this.isSeparatorLine(lines[endLine])) {
            endLine++;
        }
        ...
        if (parsed) ranges.push({ start: spans[i].start, end: spans[endLine - 1].end, tableText, parsed });
        i = endLine - 1;
    }
    return ranges;
}
...
function positionForOffset(segments, target, preferEnd) {
    ...
    for (const segment of segments) {
        ...
        if (target < next || (!preferEnd && target === offset) || (preferEnd && target === next)) {
            if (segment.type === "text") {
                return { node: segment.node, offset: Math.max(0, Math.min(segment.text.length, target - offset)), mode: "text" };
            }
            return { node: segment.node, mode: preferEnd ? "after" : "before" };
        }
        offset = next;
    }
    ...
    if (!last) return null;
    if (last.type === "text") return { node: last.node, offset: last.text.length, mode: "text" };
    return { node: last.node, mode: "after" };
}

function replaceTextRange(root, segments, range, replacement) {
    if (!document.createRange) return false;

    const start = this.positionForOffset(segments, range.start, false);
    const end = this.positionForOffset(segments, range.end, true);
    if (!start || !end) return false;

    const domRange = document.createRange();
    if (start.mode === "before") domRange.setStartBefore(start.node);
    else if (start.mode === "after") domRange.setStartAfter(start.node);
    else domRange.setStart(start.node, start.offset);
...
function processMessage(el) {
    ...
    // Replace from the end so earlier offsets stay valid. Refresh segments after each mutation.
...
function processAll() {
    for (const el of this.messageContentNodes()) {
